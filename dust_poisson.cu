...
int main(int argc, char *argv[]) {
  ...
  poisson_kernel<real_t><<<blockCount, blockSize>>>(rng_state.data(), draws, total_draws, draw_per_thread);
  // poisson_kernel<real_t><<<1, 1>>>(rng_state.data(), draws, total_draws, draw_per_thread);
